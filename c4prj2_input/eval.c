...
void compare_n_hands(deck_t** hands, hand_result_t* gameResults, int numHands) {
	//generalize compare_hands code

			//increment the win count for the winning hand (or tie count for ties)
	//store winning hand or tieing hand
	int continuingIndex = 0;
	bool gameTied = true;

	//pitting hands against one another to find if it is a tie or we have a winner
	for (int i = 1; i < numHands; i++) {
		deck_t* continuingHand = hands[continuingIndex];
		int handResult = compare_hands(continuingHand, hands[i]);

		//continuingHand moves on to compete against the next hand
		if (handResult == 1) {
			gameTied = false;
		}
		
		//hands[i] (challenging hand) wins, becomes continuingHand
	...
	if (gameTied == false) {
		for (int i = 0; i < numHands; i++) {
			if (i == continuingIndex) {
				gameResults[continuingIndex].wins++;
			}
			else {
				gameResults[i].losses++;
			}
		}
	}
...
void free_game_results(hand_result_t* gameResults, int numHands) {
	/*for (int i = 0; i < numHands; i++) {
		free(&gameResults[i]);
	}*/
	free(gameResults);
